...
def batch_accuracy(test_loader, m, num_steps, spike):
    with torch.no_grad():
        total = 0
        acc = 0
        m.eval()
        ...
        for data, targets in test_loader:
            ...
            spk_rec, _ = forward_pass(m, data, num_steps, spike=spike)
...
def main(cfg):
    ...
    if cfg.data.spike:
        train_dataloader, test_dataloader = build_dataloader_spike(
            batch_size=cfg.train.batch_size,
            path=cfg.data.root,
            num_workers=cfg.data.num_workers,
        )
    else:
        train_dataloader, test_dataloader = build_dataloader_static(
            batch_size=cfg.train.batch_size,
            path=cfg.data.root,
    ...
    for epoch in range(cfg.train.num_epochs):
        ...
        test_acc = batch_accuracy(
            test_dataloader, m, cfg.train.num_steps, spike=cfg.data.spike
        )
    ...
    spike_counter(m, train_dataloader, cfg, device, 5, spike=cfg.data.spike)
